...
import { getPage, queryDB, patchPage, plain, DB } from "./notion"
...
function drawLineItems(doc, F, lineItems) {
  const cw = [USABLE * 0.54, USABLE * 0.10, USABLE * 0.19, USABLE * 0.17]
  const headerH = mm(10)
  const y0 = doc.y
  ...
  lineItems.forEach((item, idx) => {
    ...
    const rowY = doc.y
    const alt  = idx % 2 === 1
    if (alt) fillRect(doc, MARGIN, rowY, USABLE, 1, C.alt) // placeholder

    // Description column (name + optional desc)
    const descX = MARGIN + mm(3)
    doc.font(F.bold).fontSize(9).fillColor(C.body)
      .text(item.name || "", descX, rowY + mm(3), { width: cw[0] - mm(6), lineBreak: true })
    const nameBottom = doc.y
    if (item.desc) {
      doc.font(F.reg).fontSize(8).fillColor(C.muted)
        .text(item.desc, descX, nameBottom, { width: cw[0] - mm(6), lineBreak: true })
    }
    const rowBottom = Math.max(doc.y, rowY + mm(10))

    // Fill alt row background retroactively
    if (alt) fillRect(doc, MARGIN, rowY, USABLE, rowBottom - rowY, C.alt)
    // Re-draw text on top (pdfkit renders top-to-bottom)
    if (alt) {
      doc.font(F.bold).fontSize(9).fillColor(C.body)
        .text(item.name || "", descX, rowY + mm(3), { width: cw[0] - mm(6), lineBreak: true })
      if (item.desc) {
    ...
    doc.font(F.reg).fontSize(9).fillColor(C.body)
      .text(`${qty}`, MARGIN + cw[0] + mm(1), midY, { width: cw[1] - mm(2), align: "center", lineBreak: false })

    // Unit Price
    const isDisc = item.is_discounted && Number(item.catalog_price || 0) > 0
    if (isDisc) {
      doc.font(F.reg).fontSize(7).fillColor(C.subtle)
        .text(fmtRM(item.catalog_price), MARGIN + cw[0] + cw[1] + mm(1), midY - mm(3),
          { width: cw[2] - mm(4), align: "right", lineBreak: false, strike: true })
      doc.font(F.reg).fontSize(9).fillColor(C.body)
        .text(fmtRM(price), MARGIN + cw[0] + cw[1] + mm(1), midY + mm(1),
          { width: cw[2] - mm(4), align: "right", lineBreak: false })
    } else {
      doc.font(F.reg).fontSize(9).fillColor(C.body)
        .text(fmtRM(price), MARGIN + cw[0] + cw[1] + mm(1), midY,
          { width: cw[2] - mm(4), align: "right", lineBreak: false })
    }

    // Amount
    doc.font(F.reg).fontSize(9).fillColor(C.body)
      .text(fmtRM(amt), MARGIN + cw[0] + cw[1] + cw[2] + mm(1), midY,
 ...
 * Draw totals block
 */
function drawTotals(doc, F, { total, hasDeposit, invoiceType, depositPaid, paymentBalance }) {
  doc.moveDown(0.5)
  ...
  if (hasDeposit) {
    rows.push({ label: "Deposit Due (50%)", value: fmtRM(total * 0.5), bold: false })
  }
  if (depositPaid > 0) {
    rows.push({ label: "Deposit Received", value: `– ${fmtRM(depositPaid)}`, bold: false })
  }
  if (paymentBalance > 0 && depositPaid > 0) {
    rows.push({ label: "Balance Due", value: fmtRM(paymentBalance), bold: false })
  }

  // Total row
  const totalLabel = invoiceType === "Final Payment" ? "Final Payment" : (invoiceType || "Total Quote")
  rows.push({ label: totalLabel, value: fmtRM(invoiceType === "Final Payment" ? paymentBalance || total : total), bold: true })
...
async function drawBottom(doc, F, { termsUrl, coBank, coHolder, coAcc, coPay }) {
  ...
  payFields.forEach(f => {
    doc.font(F.med).fontSize(7).fillColor(C.subtle)
      .text(tracked(f.label), payX, doc.y, { lineBreak: false })
    doc.moveDown(0.3)
    doc.font(F.reg).fontSize(8).fillColor(C.body)
      .text(f.value, payX, doc.y, { width: payW })
    doc.moveDown(0.7)
  })
}
...
export async function generateQuotationPdf(data) {
  ...
  // Header
  await drawHeader(doc, F, { logoUrl, coName, coContact, docTitle: "Quotation" })
  ...
  // Meta bar
  drawMetaBar(doc, F, [
    { label: "QUOTE NO.",   value: data.quotation_no || "—" },
  ...
  drawTotals(doc, F, {
    total,
    hasDeposit:     data.payment_terms === "50% Deposit",
    invoiceType:    null,
    depositPaid:    0,
    paymentBalance: 0,
  })
...
export async function generateInvoicePdf(data) {
  ...
  drawTotals(doc, F, {
    ...
    paymentBalance: data.payment_balance || 0,
  })
...
async function fetchLineItems(pageId, token) {
  ...
  try {
    ...
    // Expand callout/column blocks
    for (const block of [...allBlocks]) {
      if (["callout", "column_list", "column"].includes(block.type)) {
        try {
          const nb = await fetch(`https://api.notion.com/v1/blocks/${block.id}/children`, {
            headers: { Authorization: `Bearer ${token}`, "Notion-Version": "2022-06-28" }
          })
          if (nb.ok) allBlocks.push(...(await nb.json()).results)
        } catch {}
      }
    }
    ...
    for (const block of allBlocks) {
      ...
      try {
        ...
        for (const row of rows) {
          ...
          for (const rel of (rp.Product?.relation || []).slice(0, 1)) {
            try {
              const prod = await getPage(rel.id.replace(/-/g, ""), token)
...
export async function fetchQuotationData(pageId, token) {
  ...
  // Auto-assign quotation number
  const quotationNo = await assignQuotationNumber(issueDate, currentNo, token)
  if (quotationNo !== currentNo) {
    await patchPage(pageId, { [titleKey]: { title: [{ text: { content: quotationNo } }] } }, token)
  }
  ...
  // Line items
  let lineItems = await fetchLineItems(pageId, token)
  if (!lineItems.length && amount) {
    lineItems = [{ name: "Professional Services", desc: "", qty: 1, unit_price: amount }]
  }

  // Company details (Opxio)
  const { fetchCompanyDetails } = await import("./notion")
