...
function drawLineItems(doc, F, lineItems) {
  ...
  lineItems.forEach((item, idx) => {
    ...
    // Measure text height first (without drawing) to size the row
    const itemX = MARGIN + mm(3)
    const descX = MARGIN + cw[0] + mm(3)

    // Calculate row height based on content
    const itemTextH  = doc.heightOfString(item.name || "", { width: cw[0] - mm(6), fontSize: 9 })
    const descTextH  = item.desc
      ? doc.heightOfString(item.desc, { width: cw[1] - mm(6), fontSize: 8 })
      : 0
    const rowBottom = Math.max(
      rowY + mm(3) + itemTextH + mm(4),
      rowY + mm(3) + descTextH + mm(4),
      rowY + mm(10)
    )
    ...
    doc.font(F.bold).fontSize(9).fillColor(C.body)
      .text(item.name || "", itemX, rowY + mm(3), { width: cw[0] - mm(6), lineBreak: true })

    // DESCRIPTION column — description text
    if (item.desc) {
      doc.font(F.reg).fontSize(8).fillColor(C.muted)
        .text(item.desc, descX, rowY + mm(3), { width: cw[1] - mm(6), lineBreak: true })
    }
