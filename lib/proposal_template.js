...
const CSS = `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  @media print{
    ...
    /* ── Grid → block in print: Chromium ignores break-inside on display:grid ── */
    /* summary-grid stays 2-col (more compact, 3 rows vs 6 stacked); others converted to block */
    .module-list,.steps-grid,.phase2-grid,.addon-now-grid{
      display:block;
    }
    /* Each card: block with margin instead of grid gap */
    .module-item,.step-item,.phase2-item,.addon-now-item{
      display:block;
      margin-bottom:8px;
      page-break-inside:avoid;
      break-inside:avoid;
    }
    /* Summary grid: keep 2-col layout, just prevent it splitting across pages */
    .summary-grid{
      break-inside:avoid;
      page-break-inside:avoid;
    }
    ...
    /* Steps: restore side-by-side feel with inline-block */
    .step-item{
    ...
    .investment-table,.investment-table tfoot,.investment-table tbody tr:last-child{
      page-break-inside:avoid;
      break-inside:avoid;
    }
    /* Addon items: phase2 uses 3-col grid — keep as grid */
    .phase2-item{
      display:grid;
